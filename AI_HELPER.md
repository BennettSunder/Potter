...
- The highlighted editor tool is persistent UI state; keyboard shortcuts can switch that highlight, but normal selection remains available unless a move/rotate/scale/extrude transform is actively in progress.
...
- Clicking the `extrude` toolbar tool starts the modal extrusion workflow directly because there is no extrusion gizmo yet.
